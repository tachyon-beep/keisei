...
class TestSchemaV6Migration:
    def test_schema_v6_migration(self, tmp_path):
        """Create a v5 DB, set version to 5, re-init, verify v6 columns and version."""
        db_path = str(tmp_path / "migrate.db")
        # First init creates v5 schema (before our changes it would be 5,
        # but now it creates v6 -- so we manually downgrade)
        init_db(db_path)
        conn = sqlite3.connect(db_path)
        # Downgrade version to 5 to simulate a pre-v6 DB
        conn.execute("UPDATE schema_version SET version = 5")
        # Drop the new columns if they exist (simulating v5 state)
        # SQLite doesn't support DROP COLUMN before 3.35, but we can
        # just check that the migration adds them if missing.
        # Instead, let's create a truly v5-shaped DB.
        conn.commit()
        conn.close()

        # Re-init should trigger the v5->v6 migration
        init_db(db_path)
