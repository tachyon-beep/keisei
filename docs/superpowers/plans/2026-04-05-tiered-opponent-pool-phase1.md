...
| `keisei/training/demonstrator.py` | Modify | Update imports (OpponentPool → OpponentStore) |
| `keisei/training/league.py` | Delete | All contents moved to opponent_store.py |
...
| `tests/test_league.py` | Delete | Replaced by the four new test files |
...
Update `load_config` to parse the nested `[league]` section. Replace the `LeagueConfig(**raw["league"])` call with nested parsing:

```python
    league_config = None
    if "league" in raw:
        lg = dict(raw["league"])
        frontier_raw = lg.pop("frontier_static", lg.pop("frontier", {}))
        recent_raw = lg.pop("recent_fixed", lg.pop("recent", {}))
        dynamic_raw = lg.pop("dynamic", {})
        scheduler_raw = lg.pop("scheduler", lg.pop("matchmaking", {}))
...
- Delete: `keisei/training/league.py`
...
2. Add the `Role` and `EntryStatus` StrEnums at the top.
3. Extend `OpponentEntry` with the new fields (`role`, `status`, `parent_entry_id`, `lineage_group`, `protection_remaining`, `last_match_at`). Update `from_db_row` to coerce via `Role(row["role"])` and `EntryStatus(row["status"])`.
4. Rename `OpponentPool` to `OpponentStore`.
5. Remove `_evict_if_needed` and `_delete_entry` methods entirely.
6. Update `add_entry` to accept a `role` parameter (default `Role.UNASSIGNED`) and insert it into the DB.
7. Add `transaction()` context manager — acquires `_lock`, defers commit, rolls back on exception.
8. Add `clone_entry(source_entry_id, new_role, reason)` — copies checkpoint file, creates new DB row with lineage.
9. Add `retire_entry(entry_id, reason)` — sets status=retired, calls `log_transition`.
10. Add `update_role(entry_id, new_role, reason)` — changes role, calls `log_transition`.
11. Add `list_by_role(role)` — `SELECT * FROM league_entries WHERE role = ? AND status = 'active'`.
12. Add `list_entries()` — filters to `status = 'active'` only.
13. Add `log_transition(entry_id, from_role, to_role, from_status, to_status, reason)`.
14. Add `decrement_protection(entry_id)` — decrements protection_remaining by 1, floor 0.
15. Update `record_result` to also update `last_match_at` and decrement `protection_remaining` on both participants.
16. Update `load_opponent` to use `map_location="cpu"` then `.to(device)`.
17. Remove `OpponentSampler` class (it's replaced by MatchScheduler).

Key implementation details for `transaction()`:
...
Key implementation for `clone_entry`:
...
- [ ] **Step 5: Delete league.py and verify no import errors**

Delete `keisei/training/league.py`. At this point, other modules still import from it — those will break until Tasks 8-10 update them. That's expected; we'll fix imports in those tasks.
...
class TestDynamicManager:
    ...
    def test_evict_weakest_skips_protected(self, store):
        """Protected entries should not be evicted."""
        mgr = DynamicManager(store, DynamicConfig(
            slots=2, protection_matches=24, min_games_before_eviction=0,
        ))
        # Add two dynamic entries directly
        e1 = _add_entry(store, 1, role=Role.DYNAMIC, elo=900)
        ...
        # e1 is weaker but protected -> cannot evict. e2 is stronger, not eligible either
        # (both have 0 games < min_games_before_eviction=0... let's use 0 for simplicity)
        # Actually with min_games_before_eviction=0 and protection=0 on e2, e2 IS eligible
        # But e1 has protection=10, so e1 is not eligible. e2 at elo 1100 is evictable only
        # if it's the lowest among eligible. Since e2 is the only eligible, it would be evicted.
        # Let's test the intended case more carefully:
...
- `FrontierManager.review`: no-op (logs "Phase 1: review is a no-op").
- `RecentFixedManager.admit`: calls `store.add_entry(role=Role.RECENT_FIXED)`, returns the entry.
- `RecentFixedManager.review_oldest`: gets oldest active recent_fixed entry, checks promotion criteria, returns `(ReviewOutcome, entry)`.
...
- `DynamicManager.admit`: inside `store.transaction()`, calls `store.clone_entry`, then sets `protection_remaining`.
- `DynamicManager.evict_weakest`: finds lowest-Elo eligible entry, calls `store.retire_entry`.
...
class MatchScheduler:
    def __init__(self, config: MatchSchedulerConfig) -> None:
        self.config = config
        self._h2h_counts: Counter[tuple[int, int]] = Counter()
...
class TieredPool:
    ...
    def bootstrap_from_flat_pool(self) -> None:
        ...
        if not entries:
            conn.execute("UPDATE league_meta SET bootstrapped = 1 WHERE id = 1")
            conn.commit()
            return
        ...
        with self.store.transaction():
            ...
            n_dynamic = n - n_frontier - n_recent

            # Frontier: select spanning Elo range
            frontier_selected = self.frontier_manager.select_initial(entries, count=n_frontier)
            ...
            # Recent: most recent by epoch (excluding frontier)
            remaining = [e for e in entries if e.id not in frontier_ids]
            remaining.sort(key=lambda e: e.created_epoch, reverse=True)
            recent_selected = remaining[:n_recent]
            ...
            # Dynamic: next by Elo (excluding assigned)
            assigned_ids = frontier_ids | recent_ids
        ...
        logger.info(
            "Bootstrap complete: %d frontier, %d recent, %d dynamic, %d retired",
            len(frontier_selected), len(recent_selected),
            min(n_dynamic, len(dynamic_candidates)),
            max(0, len(entries) - n_frontier - n_recent - n_dynamic),
        )
```
...
- [ ] **Step 1: Verify new tests cover old test scenarios**
...
- [ ] **Step 2: Delete test_league.py**

```bash
git rm tests/test_league.py
git commit -m "chore(tests): remove test_league.py (replaced by tiered pool test suite)"
```

---

### Task 8: Update KataGoTrainingLoop Integration
...
- [ ] **Step 7: Update tournament initialization**

The tournament now takes `store` instead of creating its own connection. Update the `LeagueTournament` constructor call to pass the store (this depends on Task 9 being done first — if not done yet, leave a TODO comment).
...
### Task 9: Update Tournament Runner
...
- Replace `self._generate_round(entries)` with `self.scheduler.sample_tournament_pair(entries_by_role)` for each pairing
...
### Task 10: Update Demonstrator and Remaining Imports
...
Run: `uv run grep -r "from keisei.training.league" keisei/ tests/ --include="*.py"`
Expected: No results.

- [ ] **Step 4: Run full test suite**

Run: `uv run pytest tests/ -v --no-header 2>&1 | tail -20`
Expected: All pass.
...
Run: `uv run grep -r "from keisei.training.league" tests/ --include="*.py" -l`
...
- [ ] **Step 2: Verify no references to deleted league.py**

Run: `uv run grep -r "league\.py\|from keisei.training.league\|import league" keisei/ tests/ --include="*.py"`
Expected: No results (only docs may reference it by name).
